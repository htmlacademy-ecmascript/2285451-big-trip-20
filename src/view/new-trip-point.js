...
function createTripPointTemplate (point, city, offer) {
  const {basePrice ,dateFrom, dateTo, type, vehicleIcon, isFavorite} = point;
  const {name} = city;
  const {offers} = offer;

  const currentPointOffer = offers.map((item) =>
  ...
  const pointFavorit = isFavorite ? '--active' : '' ;

  return(`<li class="trip-events__item">
      <div class="event">
        <time class="event__date" datetime="2019-03-18">${pointData}</time>
        <div class="event__type">
          <img class="event__type-icon" width="42" height="42" src=${vehicleIcon} alt="Event type icon">
        </div>
        ...
        <button class="event__favorite-btn event__favorite-btn${pointFavorit}" type="button">
